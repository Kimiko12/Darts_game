...
def colorizing_image_in_5_shades(image, palette):
    ...
    red_shades = palette[0]
    mask = np.all(approximated_image == red_shades, axis = -1)
    
    red_mask_image = np.zeros_like(image_rgb)
    red_mask_image[mask] = red_shades
...
if __name__ == '__main__':
    ...
    colorizing_image_in_5_shades(new_image, palette)
